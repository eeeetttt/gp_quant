...
输入：experiments/ml/features.csv, models/model.pkl
...
BACKTEST_REPORT_PATH = os.path.join(RESULTS_DIR, "backtest_report.json")

TARGET_COL = "target_direction_5d"
PROB_THRESHOLD = 0.55  # 预测概率超过此值才买入
...
def backtest_model():
    ...
    df['signal'] = df['prob_up'].apply(lambda p: "BUY" if p > PROB_THRESHOLD else "HOLD")

    logger.info("BUY 信号: %d (%.1f%%)", (df['signal'] == 'BUY').sum(),
                (df['signal'] == 'BUY').mean() * 100)
    logger.info("正样本实际比例: %.1f%%", df[TARGET_COL].mean() * 100)
    ...
    for symbol in symbols:
        ...
        for i in range(len(stock_df)):
            ...
            if not in_position and signal == "BUY":
                ...
                # 强制持有最多 10 天
                days_held = i - stock_df[stock_df['date'] == entry_date].index[0] if entry_date in stock_df['date'].values else 0
                ...
                if should_sell or max_hold or i == len(stock_df) - 1:
                    ...
                    all_trades.append({
                        ...
                        "is_win": bool(is_win),
                        "days_held": int(i - stock_df[stock_df['date'] == entry_date].index[0]) if entry_date in stock_df['date'].values else 0,
                    })
    ...
    # 计算回测指标
    if total_trades == 0:
        logger.warning("没有产生任何交易，降低阈值重试...")
        return
    ...
    for month, pnl_list in sorted(monthly.items()):
        monthly_stats[month] = {
            "trades": len(pnl_list),
            "avg_pnl": round(np.mean(pnl_list), 4),
    ...
    report = {
        "strategy": "RF Model - BUY when prob_up > %.2f" % PROB_THRESHOLD,
        ...
        "avg_days_held": round(np.mean([t['days_held'] for t in all_trades]), 2),
        "monthly_stats": monthly_stats,
        "prob_analysis": prob_analysis,
        "sample_trades": all_trades[:20],  # 前 20 笔交易
    }
